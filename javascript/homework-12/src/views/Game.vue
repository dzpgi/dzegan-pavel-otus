...
export default {
  ...
  setup() {
    ...
    const doRunGame = function(start) {
      ...
      isRunningGame.value = start? start: (start === false? false: isRunningGame.value)
    ...
    const doCheckTask = function() {
      ...
      if (isRunningGame.value && result) store.commit('setCorrectTaskCnt', correctTaskCount.value)
    }
